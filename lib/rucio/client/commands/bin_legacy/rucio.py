...
def list_dids(args, client, logger, console, spinner):
    ...
    if args.csv:
        if args.short:
            print(",".join([did for did, _ in table_data]))
        else:
            for did, type_ in table_data:
                print(did, type_, sep=',')
...
def list_parent_dids(args, client, logger, console, spinner):
    ...
    if args.pfns:
        ...
        for i, pfn in enumerate(dict_datasets):
            if args.csv:
                print(pfn, *dict_datasets[pfn], sep=',')
        ...
        for i, guid in enumerate(dict_datasets):
            if args.csv or (cli_config != 'rich'):
                print(guid, ','.join(dict_datasets[guid]), sep=',')
            elif cli_config == 'rich':
                parent_tree = Tree('')
...
You can filter by key/value, e.g.::
    ...
    ls_parser.add_argument("--csv", action='store_true', help='Output as a CSVs')
...
You can filter by key/value, e.g.::
    ...
    list_parser.add_argument("--csv", action='store_true', help='Output as a CSVs')
...
::
    ...
    list_parent_parser.add_argument('--csv', action='store_true', help='Output as a csv')
