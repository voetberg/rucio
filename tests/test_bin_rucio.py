...
class TestBinRucio:
    ...
    def test_download_pfn(self):
        ...
        cmd = f'rucio -v download --dir {download_dir} --rses {self.def_rse} --pfn {replica_pfn} {self.user}:{name}'
        exitcode, out, err = execute(cmd)
        print(err)
    ...
    def test_download_timeout_options_accepted(self):
        ...
        cmd = 'rucio -v download --rses {0} --transfer-speed-timeout 1 --pfn http://a.b.c/ {1}:{2}'.format(self.def_rse, self.user, tmp_file1)
        exitcode, out, err = execute(cmd)
        print(out, err)
